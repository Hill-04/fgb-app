...
Como posso ajudar? Posso gerar confrontos, sugerir datas, criar blocos de categorias, simular cenários e muito mais.`,
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] bg-[#0B0F1E] rounded-2xl border border-fgb-ink-800/50 shadow-2xl overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-fgb-ink-800/50 bg-fgb-ink-900/20 backdrop-blur-md flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-fgb-ink-100 tracking-tight">Assistente FGB</h3>
            ...
            <p className="text-xs text-fgb-ink-500 flex items-center gap-1">
              {championshipId ? (
        ...
        <select 
          ...
          className="bg-fgb-ink-800/50 border border-fgb-ink-700 text-xs text-fgb-ink-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-fgb-yellow-500/50 transition-all cursor-pointer"
        >
      ...
      >
          ...
          <div key={i} className={cn("flex flex-col", msg.role === 'user' ? "items-end" : "items-start")}>
            <div className="flex items-center gap-2 mb-1 px-1">
               <span className={cn(
                 ...
                 msg.role === 'user' ? "text-fgb-yellow-500 order-2" : "text-purple-500"
               )}>
            ...
            <div className={cn(
              ...
              msg.role === 'user' 
                ? "bg-fgb-yellow-500/10 border-fgb-yellow-500/20 rounded-tr-none text-fgb-ink-200" 
                : "bg-fgb-ink-900/80 border-fgb-ink-700/50 rounded-tl-none text-fgb-ink-300"
            )}>
              <div className="prose prose-invert prose-sm max-w-none prose-table:border prose-table:border-fgb-ink-700 prose-th:bg-fgb-ink-800/50 prose-th:px-2 prose-td:px-2">
                <ReactMarkdown>{msg.content}</ReactMarkdown>
              </div>

              {msg.actions && msg.actions.length > 0 && (
                <div className="mt-4 space-y-2 border-t border-fgb-ink-700/50 pt-3">
            ...
            <span className="text-[10px] text-fgb-ink-500 mt-1 px-1">
        ...
        {isLoading && (
          <div className="flex items-start gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
             <div className="w-8 h-8 rounded-full bg-fgb-ink-900 border border-fgb-ink-700 flex items-center justify-center">
                <Loader2 className="w-4 h-4 text-purple-500 animate-spin" />
             </div>
             <div className="bg-fgb-ink-900/80 border border-fgb-ink-700/50 px-4 py-3 rounded-2xl rounded-tl-none flex gap-1">
                <span className="w-1.5 h-1.5 bg-fgb-ink-500 rounded-full animate-bounce [animation-delay:-0.3s]"></span>
                <span className="w-1.5 h-1.5 bg-fgb-ink-500 rounded-full animate-bounce [animation-delay:-0.15s]"></span>
                <span className="w-1.5 h-1.5 bg-fgb-ink-500 rounded-full animate-bounce"></span>
             </div>
          </div>
        )}
      </div>
      ...
      {/* Input Area */}
      <div className="p-4 border-t border-fgb-ink-800/50 bg-fgb-ink-900/40 space-y-3">
        {/* Suggestion Chips */}
        ...
        <div className="flex gap-2">
          <input
            ...
            disabled={isLoading}
            className="flex-1 bg-fgb-ink-950/50 border border-fgb-ink-800 rounded-xl px-4 py-3 text-sm text-fgb-ink-200 outline-none focus:ring-2 focus:ring-fgb-yellow-500/50 focus:border-fgb-yellow-500/50 transition-all placeholder:text-fgb-ink-600 disabled:opacity-50"
          />
          <button
