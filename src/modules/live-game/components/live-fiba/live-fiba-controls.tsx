...
function ControlButton({
  ...
  return (
    ...
    >
      <span className="inline-flex items-center gap-2 text-[12px] font-black uppercase tracking-[0.08em]">
        {icon}
        {label}
      </span>
      <span className="line-clamp-2 text-[9px] font-bold uppercase tracking-[0.12em] text-white/45">{hint}</span>
    </button>
  )
}
  ...
  if (compact) {
    return (
      <footer className="rounded-[22px] border border-white/10 bg-[#070a10]/95 p-2 text-white shadow-[0_24px_70px_rgba(0,0,0,0.42)] backdrop-blur-xl">
        ...
        <div className="grid gap-1.5 xl:grid-cols-[1.45fr_0.55fr]">
            ...
            <div className="grid min-h-[52px] gap-1 rounded-[18px] border border-white/10 bg-white/[0.05] px-3 py-2">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                ...
                <button
                  ...
                  className="rounded-xl border border-white/10 bg-white/[0.05] px-2 py-1.5 text-[9px] font-black uppercase tracking-[0.12em] text-white/78 transition hover:bg-white/[0.1]"
                >
                  14s
                </button>
              </div>
          ...
          <div className="grid gap-1.5 md:grid-cols-[1fr_auto]">
            <div className="grid gap-1 rounded-[18px] border border-white/10 bg-white/[0.04] px-3 py-2">
              ...
              <p className="text-[11px] font-black uppercase tracking-[0.05em] text-white">
            ...
            {onRefresh ? (
              <button
                type="button"
                onClick={onRefresh}
                className="inline-flex min-h-[52px] items-center justify-center gap-2 rounded-[18px] border border-white/10 bg-white/[0.05] px-4 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-white transition hover:bg-white/[0.1]"
              >
      ...
      <div className="grid gap-2 xl:grid-cols-[1.3fr_0.9fr]">
          ...
          <div className="grid min-h-[52px] gap-1 rounded-[18px] border border-white/10 bg-white/[0.05] px-3 py-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                type="button"
                onClick={() => onShotClockReset(14)}
                className="rounded-xl border border-white/10 bg-white/[0.05] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-white/78 transition hover:bg-white/[0.1]"
              >
                14s
              </button>
            </div>
        ...
        <div className="grid gap-1.5 md:grid-cols-[1fr_auto]">
          <div className="grid gap-1 rounded-[20px] border border-white/10 bg-white/[0.04] px-3 py-2">
            ...
            <p className="text-[12px] font-black uppercase tracking-[0.05em] text-white">
              {tableModel.home.shortName} {tableModel.home.score} x {tableModel.away.score} {tableModel.away.shortName}
            </p>
            ...
            >
              ...
              <p className="mt-1 truncate text-[12px] font-black uppercase tracking-[0.04em] text-white">
            ...
            {error ? <p className="text-[11px] font-semibold text-[#FFB4B7]">{error}</p> : null}
          </div>
          ...
          {onRefresh ? (
            <button
              type="button"
              onClick={onRefresh}
              className="inline-flex min-h-[52px] items-center justify-center gap-2 rounded-[18px] border border-white/10 bg-white/[0.05] px-4 py-2.5 text-[11px] font-black uppercase tracking-[0.12em] text-white transition hover:bg-white/[0.1]"
            >
              <RefreshCcw className="h-4 w-4" />
              Refresh
              <RotateCcw className="h-4 w-4" />
