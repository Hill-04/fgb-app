  ...
  if (!championship?.hasPlayoffs) {
    return (
      <div className="bg-[var(--fgb-ink-900)] border border-white/[0.08] rounded-[40px] p-16 text-center animate-in zoom-in-95 duration-700">
        ...
        <p className="text-sm text-fgb-ink-500 max-w-sm mx-auto font-medium">
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-fgb-ink-500 mt-1">
        ...
        {categories.map(cat => (
          <div key={cat.id} className="bg-[var(--fgb-ink-900)] border border-white/[0.08] rounded-[40px] p-8 overflow-hidden relative shadow-sm">
            ...
            <div className="flex items-center gap-3 mb-10">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--fgb-yellow-500)]">Categoria</span>
            ...
            {cat.standings.length === 0 ? (
              <div className="py-12 text-center bg-white/[0.02] rounded-3xl border border-white/5">
                <p className="text-xs text-fgb-ink-600 italic font-medium">Aguardando definição dos times nos standings da fase regular</p>
              </div>
            ) : (
              <div className="overflow-x-auto pb-4 custom-scrollbar">
                <div className="flex gap-12 min-w-max items-center">
                  ...
                  <div className="flex flex-col gap-6">
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-fgb-ink-600 mb-2 border-l-2 border-[var(--fgb-yellow-500)] pl-3">
                        ...
                        <div className="bg-[var(--fgb-ink-900)] border border-white/[0.06] rounded-2xl overflow-hidden w-52 shadow-lg shadow-black group">
                          ...
                          <div className="px-4 py-3 flex items-center justify-between border-b border-white/[0.04] bg-white/[0.02] group-hover:bg-white/[0.04] transition-colors">
                            <div className="flex items-center gap-3 min-w-0">
                               <span className="text-[9px] font-black text-[var(--fgb-yellow-500)] w-5">{i*2+1}°</span>
                               <span className="text-[11px] font-black text-white truncate uppercase">{pair[0]?.team?.name || 'TBD'}</span>
                            </div>
                            <span className="text-[10px] font-black text-fgb-ink-600">--</span>
                          </div>
                          {/* Segundo Time */}
                          <div className="px-4 py-3 flex items-center justify-between group-hover:bg-white/[0.02] transition-colors">
                            <div className="flex items-center gap-3 min-w-0">
                               <span className="text-[9px] font-black text-fgb-ink-600 w-5">{championship.playoffTeams - i}°</span>
                               <span className="text-[11px] font-black text-fgb-ink-400 truncate uppercase">{pair[1]?.team?.name || 'TBD'}</span>
                            </div>
                            <span className="text-[10px] font-black text-fgb-ink-800">--</span>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    {Array.from({ length: 2 }).map((_, i) => (
                      <div key={i} className="bg-[var(--fgb-ink-900)] border border-white/[0.06] rounded-2xl overflow-hidden w-52 shadow-lg shadow-black group">
                        ...
                        <div className="px-4 py-3">
                          <span className="text-[10px] font-black text-fgb-ink-600 italic uppercase tracking-widest">Vencedor Q{i*2+2}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-col justify-center gap-6">
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--fgb-yellow-500)] mb-2 border-l-2 border-[var(--fgb-yellow-500)] pl-3">
                      Grande Final
                    </p>
                    <div className="bg-gradient-to-br from-[var(--fgb-yellow-500)]/20 to-transparent border border-[var(--fgb-yellow-500)]/30 rounded-3xl overflow-hidden w-52 shadow-2xl shadow-fgb-yellow-600/10 relative">
                       <div className="absolute top-0 right-0 p-2">
                          <div className="w-2 h-2 rounded-full bg-[var(--fgb-yellow-500)] animate-pulse" />
                       </div>
                      <div className="px-4 py-5 border-b border-[var(--fgb-yellow-500)]/10">
