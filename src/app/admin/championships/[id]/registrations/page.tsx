...
export default function RegistrationsPage() {
  ...
  const registrationStatus = useMemo(() => {
    ...
    return new Date(championship.regDeadline) > now
      ? { label: 'Inscricoes Abertas', className: 'text-green-600 bg-green-50 border-green-200' }
      : { label: 'Inscricoes Encerradas', className: 'text-red-600 bg-red-50 border-red-200' }
  }, [championship])
  ...
  const handleDelete = async (registrationId: string) => {
    if (!confirm('Deseja realmente excluir esta inscricao?')) {
      return
    }
  ...
  const handleAddBlockedDate = () => {
    if (!blockedDraft.startDate || !blockedDraft.endDate) {
      setFormError('Informe data inicial e final do periodo bloqueado.')
      return
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!response.ok) {
      setFormError(data.error || 'Erro ao salvar inscricao.')
  ...
  if (loading || !championship) {
    return (
      <div className="flex min-h-[320px] items-center justify-center gap-3">
        ...
        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--gray)]">Carregando inscricoes</span>
      </div>
    )
  }
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-[var(--gray)]">
                          {registration.coachName ? `Tecnico: ${registration.coachName}` : 'Sem tecnico informado'}
                        </p>
                      </div>
                    </div>
                  </td>
        ...
        {registrations.length === 0 && (
          <div className="p-16 text-center">
            <p className="text-[10px] font-black uppercase tracking-widest text-[var(--gray)]">
              Nenhuma inscricao cadastrada ainda
            </p>
          </div>
        )}
      </div>
          ...
          <div className="max-h-[92vh] w-full max-w-4xl overflow-hidden rounded-[28px] border border-[var(--border)] bg-white shadow-2xl">
            <div className="border-b border-[var(--border)] bg-[var(--gray-l)] px-8 py-6">
              <h3 className="fgb-display text-3xl text-[var(--black)] leading-none">
                {editingReg ? 'Editar Inscricao' : 'Nova Inscricao'}
              </h3>
              <p className="fgb-label mt-1 text-[var(--gray)]" style={{ fontSize: 10, letterSpacing: 2 }}>
                Gestao completa de logistica, bloqueios e atletas multi-categoria
              </p>
            </div>
            ...
            <div className="max-h-[70vh] space-y-8 overflow-y-auto px-8 py-8">
              ...
              <section className="grid gap-6 md:grid-cols-2">
                <div className="space-y-4 rounded-[24px] border border-[var(--border)] bg-[var(--gray-l)] p-5">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[var(--gray)]">Responsavel</p>
                    <h4 className="mt-1 text-lg font-black text-[var(--black)]">Dados do tecnico</h4>
                  </div>
                  <input value={coachName} onChange={(event) => setCoachName(event.target.value)} placeholder="Nome do tecnico" className="h-11 w-full rounded-xl border border-[var(--border)] bg-white px-4 text-sm outline-none focus:border-[var(--verde)]" />
                  ...
                  <label className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-white px-4 py-3 text-sm text-[var(--gray)]">
                    ...
                    Este tecnico comanda outras equipes neste campeonato
                  </label>
                </div>

                <div className="space-y-4 rounded-[24px] border border-[var(--border)] bg-[var(--gray-l)] p-5">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[var(--gray)]">Observacoes</p>
                    <h4 className="mt-1 text-lg font-black text-[var(--black)]">Restricoes gerais da equipe</h4>
                  </div>
                  <textarea
                    value={observations}
                    onChange={(event) => setObservations(event.target.value)}
                    rows={7}
                    placeholder="Descreva ginasio, logistica, observacoes medicas ou outras restricoes relevantes."
              ...
              <section className="space-y-5 rounded-[28px] border border-[var(--border)] bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                  <div>
                    ...
                    <h4 className="mt-1 text-lg font-black text-[var(--black)]">Adicionar periodo bloqueado</h4>
                    <p className="text-sm text-[var(--gray)]">Ative a opcao abaixo se a equipe nao puder comparecer de forma alguma na data informada.</p>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  {blockedDates.map((blockedDate, index) => (
                    <div key={`${blockedDate.startDate}-${blockedDate.endDate}-${index}`} className="flex items-center justify-between rounded-2xl border border-[var(--border)] bg-[var(--gray-l)] px-4 py-3">
                      <div>
                        <p className="text-sm font-black text-[var(--black)]">{blockedDate.startDate} ate {blockedDate.endDate}</p>
            ...
            <div className="flex items-center justify-between border-t border-[var(--border)] bg-[var(--gray-l)] px-8 py-5">
              ...
              >
                {submitLoading ? 'Salvando...' : 'Salvar inscricao'}
              </button>
            </div>
          </div>
        </div>
