  ...
  return (
    <div className="space-y-10">
      <div className="animate-fade-in flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-fgb-ink-500 mb-2">
            <Link href="/admin/championships" className="hover:text-white transition-colors">Campeonatos</Link>
            <ArrowRight className="w-3 h-3" />
            <span className="text-[var(--fgb-yellow-500)]">Gerenciamento Geral</span>
          </div>
          <h1 className="text-4xl font-display font-black text-white tracking-tight">
            {championship.name}
          </h1>
          <p className="text-fgb-ink-400 font-medium mt-1">Configure categorias, valide inscricoes e organize o calendario.</p>
        </div>

        <div className="flex gap-3">
          <Link href="/admin/championships">
            <Button variant="ghost" className="h-12 px-6 rounded-xl font-bold text-fgb-ink-400 hover:text-white hover:bg-white/5 uppercase tracking-widest text-[10px]">
              ...
              <Card key={cat.id} className={`bg-[#111] border-white/5 rounded-3xl group overflow-hidden transition-all ${isViable ? 'hover:border-green-500/30' : 'hover:border-red-500/30'}`}>
                ...
                <CardContent>
                  <div className="mt-2">
                    ...
                    <div className="text-[10px] font-bold text-fgb-ink-500 uppercase tracking-widest mt-1">Equipes Inscritas</div>
                  </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between">
                    ...
                    {!isViable && (
                      <span className="text-[9px] font-medium text-fgb-ink-600">Falta {minTeams - teamCount}</span>
                    )}
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="fgb-table-wrap">
                <table className="fgb-table w-full text-left">
                  <thead className="bg-white/5 border-b border-white/5">
                    <tr>
                      <th className="px-6 py-4 text-[10px] font-bold text-fgb-ink-500 uppercase tracking-widest">Equipe</th>
                      <th className="px-6 py-4 text-[10px] font-bold text-fgb-ink-500 uppercase tracking-widest">Cidade</th>
                      <th className="px-6 py-4 text-[10px] font-bold text-fgb-ink-500 uppercase tracking-widest">Categorias</th>
                      <th className="px-6 py-4 text-[10px] font-bold text-fgb-ink-500 uppercase tracking-widest text-center">Inscricao</th>
                      <th className="px-6 py-4 text-right pr-12 text-[10px] font-bold text-fgb-ink-500 uppercase tracking-widest">Acoes</th>
                    </tr>
                  </thead>
                    ...
                    {championship.registrations.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="px-6 py-20 text-center">
                          ...
                          <p className="text-fgb-ink-500 text-sm font-medium uppercase tracking-widest">Nenhuma inscricao realizada ate o momento.</p>
                        </td>
                      </tr>
                    ) : (
                        ...
                        <tr key={reg.id} className="hover:bg-white/[0.02] transition-colors group">
                          <td className="px-6 py-5">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center font-display font-black text-[var(--fgb-yellow-500)]">
                                {reg.team.name.charAt(0)}
                              </div>
                              <span className="font-bold text-white text-sm">{reg.team.name}</span>
                            </div>
                          </td>
                          <td className="px-6 py-5 text-sm font-medium text-fgb-ink-400">{reg.team.city || 'N/A'}</td>
                          <td className="px-6 py-5">
        ...
        <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-2 gap-8">
          <Card className="bg-gradient-to-br from-[#111] to-[#0a0a0a] border-white/5 rounded-[40px] p-10 overflow-hidden relative">
            ...
            <div className="relative z-10">
              <div className="w-12 h-12 rounded-2xl bg-[var(--fgb-navy-500)]/10 flex items-center justify-center mb-6">
                <Settings2 className="w-6 h-6 text-[var(--fgb-navy-500)]" />
              </div>
              <h2 className="text-3xl font-display font-black text-white tracking-tight mb-4 leading-tight">Agrupamento e Viagens</h2>
              <p className="text-fgb-ink-500 font-medium mb-8 max-w-md">A IA ira analisar as equipes inscritas e sugerir quais categorias devem jogar juntas nas mesmas sedes para reduzir os custos em 50%.</p>
              <Link href={`/admin/championships/${championship.id}/scheduling`}>
                <Button className="bg-[var(--fgb-navy-500)] hover:bg-[var(--fgb-navy-600)] text-white font-black uppercase tracking-widest h-14 px-8 rounded-2xl shadow-xl shadow-[var(--fgb-navy-500)]/20 transition-all hover:scale-105 active:scale-95">
          ...
          <Card className="bg-[#111] border-white/5 rounded-[40px] p-10 flex flex-col justify-between">
            <div>
              ...
              <p className="text-fgb-ink-500 font-medium mb-8">Assim que a tabela estiver pronta, voce podera disparar as notificacoes e confirmar as sedes oficiais.</p>
            </div>
            <div className="flex items-center gap-6 p-6 bg-white/5 rounded-3xl border border-white/5">
              <div className="w-10 h-10 rounded-full bg-fgb-yellow-500/20 flex items-center justify-center text-[var(--fgb-yellow-500)]">
                <AlertTriangle className="w-5 h-5" />
              </div>
              <p className="text-xs font-bold text-fgb-ink-400 leading-relaxed uppercase tracking-tighter">Aguardando definicao completa das sedes pela IA para permitir a confirmacao.</p>
            </div>
          </Card>
        </div>
      </div>
