...
async function togglePublish(formData: FormData) {
  ...
  revalidatePath('/noticias')
  revalidatePath('/')
  if (post?.slug) revalidatePath(`/noticias/${post.slug}`)
}
  ...
  try {
    ...
    return (
      <div className="space-y-6 pb-12">
        <div>
          <h1 className="fgb-display text-3xl text-[var(--black)]">Noticias</h1>
          <p className="fgb-label text-[var(--gray)] mt-1" style={{ textTransform: 'none', letterSpacing: 0 }}>
            Publicacoes oficiais e comunicados.
          </p>
        </div>
        ...
        <div className="fgb-card p-5">
          <form action={createNews} className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <textarea name="content" placeholder="Conteudo" className="min-h-[120px] rounded-xl border border-[var(--border)] bg-white px-3 py-2 text-sm md:col-span-2" />
            ...
            <button type="submit" className="fgb-btn-primary h-10 rounded-xl md:col-span-2">Salvar noticia</button>
          </form>
        </div>

        <div className="fgb-card overflow-hidden">
          <div className="px-6 py-4 border-b border-[var(--border)] bg-[var(--gray-l)]">
            <p className="fgb-label text-[var(--gray)]" style={{ fontSize: 10 }}>Ultimas noticias</p>
          </div>
          <div className="divide-y divide-[var(--border)] bg-white">
            {posts.length === 0 ? (
              <div className="p-10 text-center text-sm text-[var(--gray)]">Nenhuma noticia cadastrada.</div>
            ) : (
              posts.map((post) => (
                <div key={post.id} className="p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    <p className="text-sm font-black text-[var(--black)]">{post.title}</p>
                    <p className="text-[11px] text-[var(--gray)]">{post.excerpt || 'Sem resumo'}</p>
                    <p className="text-[11px] text-[var(--gray)]">Status: {post.status}</p>
                  </div>
                  <form action={togglePublish}>
                    <input type="hidden" name="id" value={post.id} />
                    <input type="hidden" name="status" value={post.status} />
                    <button type="submit" className="fgb-btn-secondary h-9 px-4">
                      {post.status === 'PUBLISHED' ? 'Retirar do ar' : 'Publicar'}
                    </button>
                  </form>
                </div>
              ))
